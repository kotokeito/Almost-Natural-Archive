...
class TrackRenderer:
    ...
    def _draw_marker_label(self , x,y,font):
        ...
        py5.text(text_string, (x + 10) + off, (y + 19))
        py5.text(text_string, (x + 10), (y + 19) + off)
    ...
    def render(self, frame_idx, target_font):
        ...
        for h_idx, data in enumerate(self.history):
            ...
            size = py5.remap(progress, 0, 1, 4, 4) # 越老越细
            ...
            for i, (point, is_visible) in enumerate(zip(points, vis)):
                if is_visible:
                    # 坐标转换逻辑保持不变
                    trans_x = (point[0] / self.scale) *(1.5)
                    trans_y = (point[1] / self.scale)*(1.5)
...
#===============main part代码的主体部分===============
#变量==========
video_manager = VideoProcessor(r"resource\hc1_b&w_originalframe")
#video_manager_bandw = VideoProcessor(r"resource\hc1_b&w_originalframe")
...
def draw():
    ...
    if success:
        ...
        # 6. 导出（如果需要）
        py5.save_frame(r"final_output\hc1_output\hc1_output1\frame_####.png")
...
def draw_foreground_texture(textlist, frame_idx):
    ...
    string_length = py5.text_width("String")
    #py5.text("Wind",960-(wind_length/2),40)
    _draw_glowing_text("String", 960-(string_length/2), 48 , 48, ui_font)
    
    wind_length = py5.text_width("Wind")
    #py5.text("Trace",960-(trace_length/2),1070)
    _draw_glowing_text("Wind", 960-(wind_length/2),1068 , 48, ui_font)
