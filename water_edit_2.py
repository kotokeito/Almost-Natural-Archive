...
class AsciiLayer:
    ...
    def draw(self, img_source, threshold=230):
        ...
        for y in range(0, h, self.resolution):#part3
            for x in range(0, w, self.resolution):#画面中哪些范围部分要进行ASCII处理

                loc = x + y * w
                #c = mask_source.pixels[loc]
                #white = (py5.red(c) == 255 and py5.green(c) == 255 and py5.blue(c) == 255)
                #if white == True:
                    #continue
...
def draw():
    ...
    if success:
        ...
        # 6. 导出（如果需要）
        py5.save_frame(r"final_output\water_output\water1\frame_####.png")
