...
#===============main part代码的主体部分===============
#变量==========
video_manager = VideoProcessor(r"resource\green_isolated")
video_manager_edge = VideoProcessor(r"resource\edge_frames")
ascii_effect = AsciiLayer(font_size=24, resolution=8)
renderer = TrackRenderer(1280, 720, r"resource\data\tracks_coords.npy", r"resource\data\tracks_visibility.npy")
...
def draw():
    ...
    success = video_manager.update_frame(current_idx) and video_manager_edge.update_frame(current_idx)
    

    if success:
        current_frame_img = update_data(current_idx)
        current_edge_img = video_manager_edge.curr_frame_img#########
    
        # 2. 渲染背景层
        draw_background_layer(current_frame_img)
    
        # 3. 渲染原视频图层
        draw_original_video_overlay(current_edge_img)
        ...
        # 6. 导出（如果需要）
        py5.save_frame("output/frame_####.png")
...
def draw_original_video_overlay(current_frame):
    ...
    #把canny edge画上来
    py5.tint(0) #颜色设置为黑色
    #py5.tint(153, 255, 0) #这个是颜色设置为#99FF00
